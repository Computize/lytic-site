...
const Carousel = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & CarouselProps>(({ orientation = 'horizontal', autoPlay, opts, setApi, plugins, className, children, ...props }, ref) => {
  ...
  const useAutoPlay = autoPlay === true ? [Autoplay({ delay: 12000 })] : plugins;
...
const CarouselPrevious = React.forwardRef<HTMLButtonElement, React.ComponentProps<typeof Button>>(({ className, variant = 'outline', size = 'icon', ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant={variant}
      size={size}
      className={cn('absolute  h-8 w-8 rounded-full', orientation === 'horizontal' ? '-left-12 bottom-0 md:bottom-1/2  -translate-y-1/2' : '-top-12 left-1/2 -translate-x-1/2 rotate-90', className)}
...
const CarouselNext = React.forwardRef<HTMLButtonElement, React.ComponentProps<typeof Button>>(({ className, variant = 'outline', size = 'icon', ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant={variant}
      size={size}
      className={cn('absolute h-8 w-8 rounded-full', orientation === 'horizontal' ? '-right-12 bottom-0 md:bottom-1/2 -translate-y-1/2' : '-bottom-12 left-1/2 -translate-x-1/2 rotate-90', className)}
